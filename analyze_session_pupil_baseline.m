function pupil = analyze_session_pupil_baseline(video_path, tosca_path)
    ...
    for analyzeNum = 1:length(targetFiles)
        ...
        for t = 1:length(log.trials)
            ...
            if sum(choose_filtered) < 1
                ...
                pupil.diameter = [pupil.diameter; nan];
                pupil.sessionNum = [pupil.sessionNum, str2num(f{end})];
                ...
                if ~isempty(relativeTiming)
                    ...
                    %resample = [-1:1/500:3];
                    try
                        pupil_diameter = nanmean(p(relativeTiming < 0 & relativeTiming > -.5));
                    catch
